...
public class NumberGuessServletTest {
    ...
    public void testGuessTooLow() throws Exception {
        servlet.setTargetNumber(50); // Setting target number for testing
        Mockito.when(request.getParameter("guess")).thenReturn("30"); // Guess is too low
    ...
    public void testGuessTooHigh() throws Exception {
        servlet.setTargetNumber(50); // Setting target number for testing
        Mockito.when(request.getParameter("guess")).thenReturn("80"); // Guess is too high
        servlet.doPost(request, response);
